	...
	class API extends REST {
		...
		*/
		...
		private function writers(){	
			if($this->get_request_method() != "GET"){
				$this->response('',406);
                        ...
                        $query="SELECT distinct c.ID, c.email, c.catagory, c.sample, c.topic, c.myReferences, c.paygrade, c.comments FROM writerskillsample  c order by c.ID desc";
			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
		...
		private function updatewriter(){
			...
			*/
			$query = "UPDATE writerskillsample SET paygrade = '$paygrade' WHERE ID = $id ";
			if(!empty($writer)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
